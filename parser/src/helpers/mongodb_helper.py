...
class MongoClientSingleton:
    ...
    def __init__(self, db_dsn, connectTimeoutMS, socketTimeoutMS, username, password):
        self.mongo_client = MongoClient(db_dsn, connectTimeoutMS=connectTimeoutMS,
                                        socketTimeoutMS=socketTimeoutMS)
        self.mongo_client.admin.authenticate(username, password)
...
class MongodbHelper:
    def __init__(self, collection_name: str, db_dsn: str, db_name: str, username: str, password: str):
        try:
            ...
            mongo_client = MongoClientSingleton(self.db_dsn,
                                                connectTimeoutMS=app_config['mongo_db']['connectTimeoutMS'],
                                                socketTimeoutMS=app_config['mongo_db']['connectTimeoutMS'],
                                                username=username,
                                                password=password)

            self.client = mongo_client.get_mongo_client()
    ...
    def get_all(self, predicate={}, fields={}, get_all_fields: bool = False, sort: str = None, limit: int = None,
                skip: int = None, sort_type: int = -1, no_cursor_timeout: bool = False) -> object:

        if get_all_fields:
            fields = None

        result = self.get_collection().find(predicate, fields, no_cursor_timeout=no_cursor_timeout)

        if sort is not None:
            # print("sort")
            result = result.sort(sort, sort_type)
    ...
    def aggregate2(self, aggregate_list: list):
    ...
    def edit_custom(self, predicate: dict, query: dict, is_upsert: bool = False) -> dict:
        """ UpdateOne({'_id': 4}, {'$inc': {'j': 1}}, upsert=True) """
        result = self.get_collection().update_one(predicate, query, upsert=is_upsert)
        return dict(
            matched_count=result.matched_count,
            modified_count=result.modified_count,
            upserted_id=result.upserted_id
